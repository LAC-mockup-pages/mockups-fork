...
const filtering = (list, group) => {
  const result = list
    .filter((record) => record.CategoryID === group)
    .map((record) => {
      const { ReportName, CategoryID, Sort_Order2, Sort_Order } = record;
      return { ReportName, CategoryID, Sort_Order2, Sort_Order };
    });
  return result;
};

console.table(filtering(GetReport, "39"));
