...
const dataSource = Providers.slice(0);
...
const createNewRecord = (labelsList) => {
  ...
  const requiredList = ["ProviderName", "Phone", "Email"];
...
const createViewBloc = () => {
  ...
  // Sorting list of providers by descending ID
  const list = dataSource.sort(
    (provider1, provider2) => provider1.ProviderName - provider2.ProviderName
  );
